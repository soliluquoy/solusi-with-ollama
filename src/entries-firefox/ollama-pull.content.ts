export default defineContentScript({
  main(ctx) {
    const downloadModel = async (modelName: string) => {
      const ok = confirm(
        `[Solusi with Ollama Extension] Do you want to pull ${modelName} model? This has nothing to do with Ollama.com website. The model will be pulled locally once you confirm.`
      )
      if (ok) {
        alert(
    ...
    for (let i = 0; i < codeDiv.length; i++) {
      ...
      if (button && command) {
        ...
        span.title = "Download model via Solusi with Ollama"
