export default defineContentScript({
  main(ctx) {
    const downloadModel = async (modelName: string) => {
      const ok = confirm(
        `[Solusi with Ollama Extension] Do you want to pull the ${modelName} model? This has nothing to do with the huggingface.co website. The model will be pulled locally once you confirm. Make sure Ollama is running.`
      )
      if (ok) {
        alert(
          `[Solusi with Ollama Extension] Pulling ${modelName} model. For more details, check the extension icon.`
        )
